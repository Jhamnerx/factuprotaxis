                            ...
                            <template>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        {{ totals.acum_unit_gain }}
                                    </td>
                                    <td class="text-center">
                                        {{ totals.acum_overall_profit }}
                                    </td>
                                </tr>
                            </template>
                        </tfoot>
...
export default {
    ...
    computed: {
        ...
        betweenDatePicker() {
            return ["between_dates", "between_months"].includes(
                this.form.period
            );
        }
    },
    ...
    methods: {
        ...
        getTotals(records) {
            ...
            this.totals.acum_unit_gain = records
                .reduce((accum, item) => accum + Number(item.unit_gain), 0)
                .toFixed(2);
            this.totals.acum_overall_profit = records
                .reduce((accum, item) => accum + Number(item.overall_profit), 0)
                .toFixed(2);
        },
        clickDownload(type) {
        ...
        searchRemoteCategories(input) {
            if (input.length > 0) {
                ...
                this.$http
                    .get(`/categories/search-data?input=${input}`)
                    .then(response => {
