            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl p-6">
                <form method="GET" action="{{ route('taxis.propietario.permisos') }}" id="filterForm">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Tipo de Permiso</label>
                    ...
                    <div class="flex items-center space-x-2 mt-4">
                        ...
                        @if(request()->hasAny(['search', 'estado', 'tipo_permiso', 'vehiculo_id']))
                            <a href="{{ route('taxis.propietario.permisos') }}" class="btn border-gray-200 dark:border-gray-700/60 hover:border-gray-300 dark:hover:border-gray-600 text-gray-800 dark:text-gray-300">
                                <span>Limpiar</span>
                            </a>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-2">
                            ...
                            <span class="text-sm text-blue-700 dark:text-blue-300">
                                ...
                                @if(request('tipo_permiso')) Tipo: "{{ request('tipo_permiso') }}" @endif
                                @if(request('vehiculo_id'))
                                    @php $selectedVehiculo = $vehiculos->find(request('vehiculo_id')); @endphp
                                    @if($selectedVehiculo) Vehículo: {{ $selectedVehiculo->placa }} @endif
                                @endif
                            </span>
                        </div>
        ...
        @if($permisos->count() > 0)
            <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
                @foreach($permisos as $permiso)
                    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl p-6 border border-gray-200 dark:border-gray-700/60 hover:shadow-lg transition-shadow duration-200">
                        ...
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    <h3 class="font-semibold text-gray-900 dark:text-gray-100">{{ $permiso->tipo_permiso }}</h3>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Permiso #{{ $permiso->id }}</p>
                                </div>
                            </div>
                            @php
                                $estadoClasses = [
                                    'pendiente' => 'bg-yellow-100 text-yellow-800 dark:bg-yellow-500/20 dark:text-yellow-400',
                                    'aprobado' => 'bg-green-100 text-green-800 dark:bg-green-500/20 dark:text-green-400',
                            ...
                            @endphp
                            <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full {{ $estadoClasses[$permiso->estado] ?? 'bg-gray-100 text-gray-800 dark:bg-gray-500/20 dark:text-gray-400' }}">
                        ...
                        @if($permiso->datosVehiculo)
                            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 mb-4">
                                <div class="flex items-center justify-between mb-2">
                                    ...
                                    <span class="text-sm font-bold text-gray-900 dark:text-gray-100">{{ $permiso->datosVehiculo->placa }}</span>
                                </div>
                                <div class="text-xs text-gray-500 dark:text-gray-400">
                                    {{ $permiso->datosVehiculo->marca->nombre ?? 'N/A' }} {{ $permiso->datosVehiculo->modelo->nombre ?? '' }}
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="space-y-3 mb-6">
                                ...
                                <div>
                                    ...
                                    <div class="mt-2 space-y-1">
                                        @foreach($permiso->personas_autorizadas as $persona)
                                            <div class="flex items-center text-xs text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-600/50 rounded px-2 py-1">
                            ...
                            <div class="flex items-center justify-between text-xs text-gray-400 dark:text-gray-500 pt-2 border-t border-gray-200 dark:border-gray-600">
                                <span>Emitido: {{ \Carbon\Carbon::parse($permiso->created_at)->format('d/m/Y H:i') }}</span>
                                @if($permiso->creator)
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl p-8 text-center">
                <div class="max-w-md mx-auto">
                    ...
                    @else
                        <!-- Default empty state -->
                        <svg class="mx-auto h-16 w-16 text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V4a2 2 0 114 0v2m-4 0a2 2 0 104 0m-4 0V4a2 2 0 014 0v2"></path>
                        </svg>
                        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">
