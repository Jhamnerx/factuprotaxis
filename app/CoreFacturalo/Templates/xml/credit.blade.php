@php
    ...
    $document_type_id = $note->affected_document
        ? $note->affected_document->document_type_id
        : $note->data_affected_document->document_type_id;
...
<CreditNote xmlns="urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2"
    ...
    <cac:TaxTotal>
        ...
        @if ($document->total_other_taxes > 0)
            <cac:TaxSubtotal>
                ...
                <cbc:TaxAmount currencyID="{{ $document->currency_type_id }}">{{ $document->total_base_other_taxes }}
                </cbc:TaxAmount>
                <cac:TaxCategory>
                    <cac:TaxScheme>
                        <cbc:ID>9999</cbc:ID>
        ...
        @if ($document->total_plastic_bag_taxes > 0)
            <cac:TaxSubtotal>
                <cbc:TaxAmount currencyID="{{ $document->currency_type_id }}">{{ $document->total_plastic_bag_taxes }}
                </cbc:TaxAmount>
                <cac:TaxCategory>
                    <cac:TaxScheme>
                        <cbc:ID>7152</cbc:ID>
    ...
    <cac:LegalMonetaryTotal>
        <cbc:LineExtensionAmount currencyID="{{ $document->currency_type_id }}">
            {{ $note->note_credit_type_id === '13' ? '0.00' : $document->total_value }}</cbc:LineExtensionAmount>
        <cbc:TaxInclusiveAmount currencyID="{{ $document->currency_type_id }}">
            {{ $note->note_credit_type_id === '13' ? '0.00' : $document->total }}</cbc:TaxInclusiveAmount>
        ...
        <cbc:PayableAmount currencyID="{{ $document->currency_type_id }}">
            {{ $note->note_credit_type_id === '13' ? '0.00' : $document->total }}</cbc:PayableAmount>
    </cac:LegalMonetaryTotal>
    @foreach ($document->items as $row)
        <cac:CreditNoteLine>
            <cbc:ID>{{ $loop->iteration }}</cbc:ID>
            <cbc:CreditedQuantity unitCode="{{ $row->item->unit_type_id }}">{{ $row->quantity }}
            </cbc:CreditedQuantity>
            {{-- @if ($note->note_credit_type_id === '13' && $document->payment_condition_id === '02') --}}
            @if (
                ($note->note_credit_type_id === '13' && $document->payment_condition_id === '02') ||
                    $note->note_credit_type_id === '03')
