@php
    ...
    $total_discount_no_base =
        $document_xml_service->getGlobalDiscountsNoBase($document) +
        $document_xml_service->getItemsDiscountsNoBase($document);
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
    ...
    @if ($document->prepayments)
        @foreach ($document->prepayments as $prepayment)
            <cac:PrepaidPayment>
                ...
                <cbc:PaidAmount currencyID="{{ $document->currency_type_id }}">{{ $prepayment->total }}
                </cbc:PaidAmount>
            </cac:PrepaidPayment>
        @endforeach
    @endif
    @if ($document->charges)
        @foreach ($document->charges as $charge)
            <cac:AllowanceCharge>
                ...
                <cbc:MultiplierFactorNumeric>{{ $document->generalApplyNumberFormat($charge->factor, 5) }}
    ...
    <cac:TaxTotal>
        ...
        @elseif(collect($document->prepayments)->count() > 0 &&
                collect($document->discounts)->where('discount_type_id', '04')->count() === 1 &&
                $document->total_taxed >= 0)
            <cac:TaxSubtotal>
                <cbc:TaxableAmount currencyID="{{ $document->currency_type_id }}">{{ $document->total_taxed }}
                </cbc:TaxableAmount>
                <cbc:TaxAmount currencyID="{{ $document->currency_type_id }}">{{ $document->total_igv }}
        ...
        @elseif(collect($document->prepayments)->count() > 0 &&
                collect($document->discounts)->where('discount_type_id', '06')->count() === 1 &&
                $document->total_unaffected >= 0)
            <cac:TaxSubtotal>
                <cbc:TaxableAmount currencyID="{{ $document->currency_type_id }}">{{ $document->total_unaffected }}
                </cbc:TaxableAmount>
                <cbc:TaxAmount currencyID="{{ $document->currency_type_id }}">0.00</cbc:TaxAmount>
        ...
        @elseif(collect($document->prepayments)->count() > 0 &&
                collect($document->discounts)->where('discount_type_id', '05')->count() === 1 &&
                $document->total_exonerated >= 0)
            <cac:TaxSubtotal>
                <cbc:TaxableAmount currencyID="{{ $document->currency_type_id }}">{{ $document->total_exonerated }}
                </cbc:TaxableAmount>
                <cbc:TaxAmount currencyID="{{ $document->currency_type_id }}">0.00</cbc:TaxAmount>
        ...
        @if ($document->total_free > 0)
            ...
            @else --}}
                ...
                {{-- @endif --}}
                <cbc:TaxAmount currencyID="{{ $document->currency_type_id }}">{{ $document->total_igv_free }}
                </cbc:TaxAmount>
                <cac:TaxCategory>
                    <cac:TaxScheme>
                        <cbc:ID>9996</cbc:ID>
    ...
    <cac:LegalMonetaryTotal>
        ...
        @if ($tot_charges > 0)
            <cbc:TaxInclusiveAmount currencyID="{{ $document->currency_type_id }}">
                {{ $document->total - $tot_charges }}</cbc:TaxInclusiveAmount>
        @else
            <cbc:TaxInclusiveAmount currencyID="{{ $document->currency_type_id }}">{{ $document->subtotal }}
        ...
        @if ($total_discount_no_base > 0)
            <cbc:AllowanceTotalAmount currencyID="{{ $document->currency_type_id }}">{{ $total_discount_no_base }}
            </cbc:AllowanceTotalAmount>
        @endif
        @if ($document->total_charge > 0)
            <cbc:ChargeTotalAmount currencyID="{{ $document->currency_type_id }}">{{ $document->total_charge }}
        ...
        <cac:InvoiceLine>
            ...
            @if ($document->detraction && $invoice->operation_type_id == '1004')
                <cac:Delivery>
                    ...
                    <cac:DeliveryTerms>
                        <cbc:ID>02</cbc:ID>
                        <cbc:Amount currencyID="PEN">{{ $document->detraction->reference_value_effective_load }}
                        </cbc:Amount>
                    </cac:DeliveryTerms>
                    <cac:DeliveryTerms>
                        <cbc:ID>03</cbc:ID>
