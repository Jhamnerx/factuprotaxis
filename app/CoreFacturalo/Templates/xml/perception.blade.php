...
<Perception xmlns="urn:sunat:names:specification:ubl:peru:schema:xsd:Perception-1"
    ...
    <cac:ReceiverParty>
        <cac:PartyIdentification>
            <cbc:ID schemeID="{{ $document->customer->identity_document_type_id }}">{{ $document->customer->number }}
            </cbc:ID>
        </cac:PartyIdentification>
        <cac:PartyLegalEntity>
            <cbc:RegistrationName>
    ...
    @foreach ($document->documents as $doc)
        <sac:SUNATPerceptionDocumentReference>
            ...
            <cbc:TotalInvoiceAmount currencyID="{{ $doc->currency_type_id }}">{{ $doc->total_document }}
            </cbc:TotalInvoiceAmount>
            @if ($doc->payments)
                @foreach ($doc->payments as $payment)
                    <cac:Payment>
                        <cbc:ID>{{ $loop->iteration }}</cbc:ID>
                        <cbc:PaidAmount currencyID="{{ $payment->currency_type_id }}">{{ $payment->total_payment }}
                        </cbc:PaidAmount>
            ...
            @if ($doc->total_perception && $doc->total_payment && $doc->date_of_perception)
                <sac:SUNATPerceptionInformation>
                    <sac:SUNATPerceptionAmount currencyID="PEN">{{ $doc->total_perception }}
                    ...
                    @if ($doc->exchange_rate)
                        <cac:ExchangeRate>
                            <cbc:SourceCurrencyCode>{{ $doc->exchange_rate->currency_type_id_source }}
                            </cbc:SourceCurrencyCode>
                            <cbc:TargetCurrencyCode>{{ $doc->exchange_rate->currency_type_id_target }}
