@php
    ...
    if (
        !empty($configurationInPdf->show_bank_accounts_in_pdf) &&
    ...
    @else
        <div class="item_watermark" style="position: absolute; text-align: center; top:42%;">
            <img style="width: 100%" height="200px"
                ...
                alt="{{ $company->name }}" alt="anulado" class="" style="opacity: 0.1;width: 95%">
        </div>
    @endif
        ...
        <table class="full-width">
            <tr>
                <td style="width: 100%;text-align: center">
                    <span style="color: red; font-weight: bold; font-size: 1.6rem;">Esta factura es solo de
                        prueba</span>
                </td>
            </tr>
        </table>
            ...
            <tr>
                <td class="p-1 text-left align-top desc cell-solid" colspan="3" rowspan="6">
                    ...
                    @if ($document->retention)
                        ...
                        <br><span class="">Valor total del comprobante:
                        </span>{{ $document->currency_type->symbol }}
                        ...
                        <br><span class="">Porcentaje de la retención:
                        </span>{{ $document->retention->percentage * 100 }}%
                        <br><span class="">Monto de la retención
                            {{ $document->currency_type->id == 'USD' ? 'soles' : '' }}:</span>
                        ...
                        @if ($document->currency_type->id == 'USD')
                            ...
                            {{ number_format($document->retention->amount_pen / $document->exchange_rate_sale, 2) }}
                        @endif
                    @endif
                    @if ($document->detraction)
                        <span class="font-bold">N. Cta. detracciones:</span> {{ $document->detraction->bank_account }}
                        @inject('detractionType', 'App\Services\DetractionTypeService')
                        <br><span class="font-bold">B/S sujeto a detracción:</span>
                        {{ $document->detraction->detraction_type_id }} -
                        {{ $detractionType->getDetractionTypeDescription($document->detraction->detraction_type_id) }}
                        <br><span class="font-bold">Método de pago:</span>
                        {{ $detractionType->getPaymentMethodTypeDescription($document->detraction->payment_method_id) }}
                        <br><span class="font-bold">Porcentaje detracción:</span>
                        {{ $document->detraction->percentage }}%
                        <br><span class="font-bold">Monto detracción
                            {{ $document->currency_type->id == 'USD' ? 'soles' : '' }}:</span> S/
                        {{ $document->detraction->amount }}

                        @if ($document->currency_type->id == 'USD')
                            <br><span class="font-bold">Monto detracción dólares:</span>
                            {{ $document->currency_type->symbol }}
                            {{ number_format($document->detraction->amount / $document->exchange_rate_sale, 2) }}
                        @endif
