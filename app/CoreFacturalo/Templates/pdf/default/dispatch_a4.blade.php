            ...
            <thead>
                <tr>
                    <th class="border-bottom text-left">
                        {{ $document['transfer_reason_type_id'] != '02' ? 'DESTINATARIO' : 'PROVEEDOR' }}</th>
                </tr>
            </thead>
            <tbody>
                <tr>
        ...
        <tbody>
                ...
                <td colspan="2">
                    ...
                    @php
                        $direction_label_origin =
                            $document['transfer_reason_type_id'] != '02' ? 'P.Partida:' : 'P.Llegada:';
                    @endphp
                    {{ $direction_label_origin }}
                    {{ $establishment->address !== '-' ? $establishment->address : '' }}
                    {{ $establishment->district_id !== '-' ? ', ' . $establishment->district->description : '' }}
            ...
            <tr>
                ...
                @if ($document->date_delivery_to_transport)
                    <td>Fecha de entrega de bienes al Transportista:
                        {{ $document->date_delivery_to_transport->format('Y-m-d') }}</td>
                @endif
            </tr>
        </tbody>
    </table>
                ...
                @if (!$document->is_transport_m1l)
                    <tr>
                        @if ($document->transport_data)
                            <td>Número de placa del vehículo Principal: {{ $document->transport_data['plate_number'] }}
                            </td>
                        @endif
                    ...
                    <tr>
                        @if ($document->secondary_license_plates)
                            @if ($document->secondary_license_plates->semitrailer)
                                <td>Número de placa semirremolque:
                                    {{ $document->secondary_license_plates->semitrailer }}</td>
                            @endif
                        @endif
