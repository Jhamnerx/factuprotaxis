                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <el-checkbox
                                ...
                                >¿Es comproasdasdbante de
                                contigencia?</el-checkbox
                            >
                        </div>
                    </div>
                </header>
...
export default {
    ...
    data() {
        return {
            formStage: {
                customer: { username: "Homata02", password: "87654321*" },
                fileName: "20100070031-01-FQA1-00000002.json",
                fileContent: ""
            },
            recordItem: null,
            resource: "documents",
    ...
    methods: {
        ...
        generateFact() {
            const fact = {};
            let type =
                this.form.document_type_id == "01" ? "factura" : "boleta";
            fact[type] = {
                IDE: this.getIde(),
                EMI: this.getEmi(),
                REC: this.getRec(),
        ...
        document_payment() {
            if (this.form.document_type_id == "03" && !this.is_receivable) {
                this.$http
                    .post(`/document_payments`, this.form_payment)
                    .then(response => {
                        if (response.data.success) {
                        } else {
                            this.$message.error(response.data.message);
