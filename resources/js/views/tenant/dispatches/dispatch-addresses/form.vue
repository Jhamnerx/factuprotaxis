        ...
        <form autocomplete="off" @submit.prevent="submit" v-loading="loading">
            <div class="form-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div
                            :class="{ 'has-danger': errors.establishment_code }"
                            class="form-group"
                        >
                            <label class="control-label"
                                >Código de sucursaaal</label
                            >
                            <el-input
                                v-model="form.establishment_code"
                            ></el-input>
                            <small
                                v-if="errors.establishment_code"
                                class="invalid-feedback"
                                v-text="errors.establishment_code[0]"
                            ></small>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        async clickSearch() {
            ...
            await this.$http
                .get(`/dispatch_addresses/search/${this.form.person_id}`)
                .then(response => {
        ...
        cleanDataComponents() {
            if (this.$refs.search_persons)
                this.$refs.search_persons.cleanValue();
        }
    }
};
</script>
