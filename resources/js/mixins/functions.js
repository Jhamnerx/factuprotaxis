    ...
    methods: {
        ...
        searchServiceNumber() {
            return new Promise(resolve => {
                ...
                this.$http
                    .get(
                        `/services/${identity_document_type_name}/${
                            this.form.number
                        }`
                    )
                    .then(response => {
    ...
    methods: {
        ...
        validateExchangePoints() {
            if (!this.hasPointsAvailable()) {
                return {
                    success: false,
                    message: `El total de puntos a canjear excede los puntos acumulados: ${
                        this.calculate_customer_accumulated_points
                    } puntos`
                };
            }
    ...
    methods: {
        ...
        fnIsRestrictedForSale(item, document_type_id) {
            return (
                ...
                (item != undefined && item.restrict_sale_cpe)
            );
        },
            ...
            await this.$http
                ...
                .then(response => {
                    if (!updated) {
                        ...
                        if (current_cols !== undefined)
                            this.columns = current_cols;
                    }
                })
                .catch(error => {
                    console.error(error);
...
export const fnItemSearchQuickSale = {
    ...
    computed: {
        showSearchItemsMainForm() {
            return (
                this.configuration && this.configuration.search_items_main_form
            );
        }
    },
    methods: {
